...
L_0_1 = zeros(N_L,1);
for i= 1:N_L
    L_0_1(i,1) = sqrt(1-a^2)*(-a)^(i-1);
end

L_0_2 = zeros(N_L,1);
for i= 1:N_L
    L_0_2(i,1) = sqrt(1-a^2)*(-a)^(i-1);
end

L_0_3 = zeros(N_L,1);
for i= 1:N_L
    L_0_3(i,1) = sqrt(1-a^2)*(-a)^(i-1);
end

L_0_4 = zeros(N_L,1);
for i= 1:N_L
    L_0_4(i,1) = sqrt(1-a^2)*(-a)^(i-1);
end

L_0 = [ L_0_1 , zeros(N_L,1), zeros(N_L,1), zeros(N_L,1);
        zeros(N_L,1) , L_0_2,zeros(N_L,1),zeros(N_L,1) ;
        zeros(N_L,1),zeros(N_L,1), L_0_3 , zeros(N_L,1);
        zeros(N_L,1), zeros(N_L,1), zeros(N_L,1),L_0_4];
...
for i = 1:N_L
    A_L_1(i,i) = a;
    for j= 1:i-1
        A_L_1(i,j) = sqrt(1-a^2)*(((-1)^(i-j+1))*a^(i-j-1));
    end
end
...
for i = 1:N_L
    A_L_2(i,i) = a;
    for j= 1:i-1
        A_L_2(i,j) = sqrt(1-a^2)*(((-1)^(i-j+1))*a^(i-j-1));
    end
end
...
for i = 1:N_L
    A_L_3(i,i) = a;
    for j= 1:i-1
        A_L_3(i,j) = sqrt(1-a^2)*(((-1)^(i-j+1))*a^(i-j-1));
    end
end
A_L_4 =zeros(N_L,N_L);
...
for i = 1:N_L
    A_L_4(i,i) = a;
    for j= 1:i-1
        A_L_4(i,j) = sqrt(1-a^2)*(((-1)^(i-j+1))*a^(i-j-1));
    end
end


A_L = [A_L_1 , zeros(N_L,N_L),zeros(N_L,N_L), zeros(N_L,N_L);
       zeros(N_L,N_L) , A_L_2, zeros(N_L,N_L), zeros(N_L,N_L);
       zeros(N_L,N_L), zeros(N_L,N_L), A_L_3,zeros(N_L,N_L);
       zeros(N_L,N_L),zeros(N_L,N_L),zeros(N_L,N_L),A_L_4];    
    
L_ = zeros(4*N_L,N*4);
L_(:,1:4) = L_0;
for i = 1:N-1
    L_(:,(i*4)+1:(i*4)+4) = A_L*L_(:,((i-1)*4)+1:((i-1)*4)+4);
end
...
eta = SX.sym('eta',N_L*4,1);n_controls = length(eta);
...
for k = 1:N
    st = X(:,k);  con = U(:,1);
    st_next  = A_main*st+ B_main*(L_(:,(4*(k-1))+1:(4*(k-1))+4))'*con;
    X(:,k+1) = st_next;
end
...
for k = 1:N
    st = X(:,k);  con = U(:,1);
    obj = obj+(st-P((Number_of_agent*3*k)+1:((Number_of_agent*3)*(k+1))))'*Q*(st-P(((Number_of_agent*3)*k)+1:((Number_of_agent*3)*(k+1)))) + (con)'*(L_(:,(4*(k-1))+1:(4*(k-1))+4))* R *(L_(:,(4*(k-1))+1:(4*(k-1))+4))'*(con) ; % calculate obj
